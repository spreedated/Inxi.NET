...
Public Class Inxi
    ...
    ''' </summary>
    Public Sub New()
        Me.New("/usr/bin/inxi", "/usr/bin/cpanel_json_xs", [Enum].GetValues(GetType(InxiParseFlags)).Cast(Of Integer)().Sum)
    End Sub

    ''' <summary>
    ''' Intializes the new instance of Inxi class and parses hardware
    ''' </summary>
    Public Sub New(ParseFlags As InxiParseFlags)
        Me.New("/usr/bin/inxi", "/usr/bin/cpanel_json_xs", ParseFlags)
    End Sub
    ...
    Public Sub New(InxiPath As String, CpanelJsonXsPath As String, ParseFlags As InxiParseFlags)
        If IsUnix() Then
            Debug("Inxi.NET {0} running on Unix.", GetExecutingAssembly().GetName().Version.ToString())
            Debug("Inxi parse flags: {0}", ParseFlags)
            If IsMacOS() Then
                Debug("Type: macOS")
                Hardware = New HardwareInfo(InxiPath, ParseFlags)
            Else
                Debug("Type: Unix")
...
''' <summary>
''' Select what components to parse
''' </summary>
Public Enum InxiParseFlags
    ''' <summary>
    ''' BIOS
    ''' </summary>
    BIOS = 1
